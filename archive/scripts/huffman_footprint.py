...
def estimate_huffman_storage_bytes(model: nn.Module) -> int:
    ...
    for name, t in sd.items():
        ...
        freqs_bytes = len(
            pickle.dumps(compressed["freqs"], protocol=pickle.HIGHEST_PROTOCOL)
        )
        shape_bytes = len(
            pickle.dumps(compressed["shape"], protocol=pickle.HIGHEST_PROTOCOL)
        )
...
def reconstruct_model_from_huffman(
    pruned_model: nn.Module, device: torch.device
) -> nn.Module:
    """

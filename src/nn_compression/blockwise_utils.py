...
from .tensor_bytes_utils import to_fp32_bytes, from_fp32_bytes
...
def block_shape(in_features: int, out_features: int, block_size: int, block_idx: int) -> tuple[int, int]:
    """
    ...
    if start >= out_features:
        raise ValueError("block_idx is out of range for the given out_features and block_size.")
    
    block_out = min(block_size, out_features - start)
    return (block_out, in_features)


def compress_weight_blocks_fp32(W: torch.Tensor, *, block_size: int, zstd_level: int) -> list[bytes]:
    """
...
def decompress_weight_block_fp32(entry: dict, block_idx: int) -> torch.Tensor:
    ...
    if not isinstance(W_blocks, list):
        raise ValueError("Entry must contain 'W_blocks_zstd' as a list of compressed blocks.")
    if block_idx < 0 or block_idx >= len(W_blocks):
        raise ValueError("block_idx is out of range for the number of blocks in 'W_blocks_zstd'.")
